...
export default {
  ...
  computed: {
    products() {
      return this.productsData ? this.productsData.map((product) => ({
        ...product,
        image: product.colors[0].gallery ? product?.colors[0]?.gallery[0].file.url : `${NO_IMAGE}`,
      }))
        : [];
    },
  },
  methods: {
    async loadProducts() {
      ...
      this.productsData = await response.data.items;
    },
  },
  created() {
    this.loadProducts();
